  ...
  <div>
    <div class=" mx-auto max-w-3xl mt-10 p-4">
      <div class="flex justify-between items-center">
        <p class=" font-bold text-2xl">Q{{$route.params.problemId}} - 测试结果</p>
  ...
  methods: {
    async send() {
      ...
      for (let i = 1; i <= size; i++) {
        if (stopCode) break;
        await _this.axios.post('./api/problem/judge',{
          answerId: Number(_this.$store.state.problem.standard),
          test: _this.$store.state.problem.test,
          generator: _this.$store.state.problem.generator,
          index: i
        }).then(response => {
